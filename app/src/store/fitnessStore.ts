  ...
  persist(
    (set) => ({
      ...
      importHCGymSession: (s) =>
        set(st => {
          if (st.gymSessions.some(g => g.id === s.id)) return st;
          return { gymSessions: [s, ...st.gymSessions].sort((a, b) => b.date.localeCompare(a.date)) };
        }),
      ...
      importHCSportSession: (s) =>
        set(st => {
          if (st.sportSessions.some(g => g.id === s.id)) return st;
          return { sportSessions: [s, ...st.sportSessions].sort((a, b) => b.date.localeCompare(a.date)) };
        }),
    }),
    {
      name: 'basil-fitness',
