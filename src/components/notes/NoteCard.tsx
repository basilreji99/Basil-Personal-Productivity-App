      ...
      <ul className="space-y-0.5 mt-1">
        {lines.map((line, i) => (
          <li key={i} className="flex items-start gap-1.5">
            <span className="font-work-sans text-xs text-on-surface-variant shrink-0 mt-0.5">
              {type === 'bullets' ? '•' : `${i + 1}.`}
            </span>
            <span className="font-work-sans text-xs text-on-surface-variant">{line}</span>
          </li>
        ))}
        {remaining > 0 && (
          <li className="font-inter text-[10px] text-outline">+{remaining} more</li>
      ...
      <>
        ...
        >
          ...
          <button
            ...
            className={`p-1.5 rounded-lg transition-all shrink-0 ${note.pinned ? 'text-primary' : 'text-on-surface-variant opacity-40 hover:opacity-100 active:opacity-100'}`}
          >
      ...
      <>
        ...
        >
          ...
          <div className="flex items-center gap-1 px-2 pb-2">
            <button
              onClick={(e) => { e.stopPropagation(); onPin(note.id); }}
              className={`p-1 rounded-lg hover:bg-surface-container active:bg-surface-container transition-colors ${note.pinned ? 'text-primary' : 'text-on-surface-variant opacity-50 hover:opacity-100 active:opacity-100'}`}
            >
              <span className={`material-symbols-outlined text-[14px] ${note.pinned ? 'icon-fill' : ''}`}>push_pin</span>
            </button>
            <button
              onClick={(e) => { e.stopPropagation(); setConfirmDelete(true); }}
              className="p-1 rounded-lg hover:bg-error-container/20 active:bg-error-container/20 text-on-surface-variant opacity-50 hover:opacity-100 active:opacity-100 hover:text-error transition-colors"
            >
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center gap-1 px-3 pb-3">
          <button
            onClick={(e) => { e.stopPropagation(); onPin(note.id); }}
            className={`p-1.5 rounded-lg hover:bg-surface-container active:bg-surface-container transition-colors ${note.pinned ? 'text-primary' : 'text-on-surface-variant opacity-40 hover:opacity-100 active:opacity-100'}`}
          ...
          <button
            onClick={(e) => { e.stopPropagation(); setConfirmDelete(true); }}
            className="p-1.5 rounded-lg hover:bg-error-container/20 active:bg-error-container/20 text-on-surface-variant opacity-40 hover:opacity-100 active:opacity-100 hover:text-error transition-colors"
            title="Delete"
          >
