...
function OAuthHandler() {
  ...
  useEffect(() => {
    if (Capacitor.isNativePlatform()) {
      ...
      const listenerPromise = CapApp.addListener('appUrlOpen', ({ url }) => {
        Browser.close().catch(() => {});
        // Robust query-param extractor — new URL() can fail on long reverse-scheme URIs
        ...
        // Handles both basilapp://oauth2callback?code=XXX and com.googleusercontent.apps.XXX:/oauth2redirect?code=XXX
  ...
  // Auto-activate/complete sprints based on today's date
  useEffect(() => {
    useSprintStore.getState().autoActivateSprints();
    // Re-check daily while the app is open
    const id = setInterval(() => useSprintStore.getState().autoActivateSprints(), 60 * 60 * 1000);
