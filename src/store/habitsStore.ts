...
interface HabitsState {
  ...
  markSourceCompleted: (source: 'gym' | 'sports', date: string) => void;
}
    ...
    (set, get) => ({
      ...
      addHabit: (partial) => {
        const habit: Habit = {
          ...
          linkedSource: partial.linkedSource,
      ...
      getLongestStreak: (habitId) => {
        const habit = get().habits.find((h) => h.id === habitId);
        if (!habit) return 0;
        const completedEntries = get().entries.filter((e) => e.habitId === habitId && e.completed);
        if (completedEntries.length === 0) return 0;
        const dates = completedEntries.map((e) => e.date).sort();

        if (habit.frequency === 'weekly') {
          const first = getWeekStart(new Date(dates[0] + 'T12:00:00'));
          const last  = getWeekStart(new Date(dates[dates.length - 1] + 'T12:00:00'));
          let longest = 0; let current = 0;
          let week = new Date(first);
          while (week <= last) {
            if (get().getWeekCompletionCount(habitId, week) >= habit.targetDays) {
        ...
        if (habit.frequency === 'monthly') {
          const fd = new Date(dates[0] + 'T12:00:00');
          const ld = new Date(dates[dates.length - 1] + 'T12:00:00');
          let longest = 0; let current = 0;
          let y = fd.getFullYear(); let m = fd.getMonth();
          const ly = ld.getFullYear(); const lm = ld.getMonth();
          while (y < ly || (y === ly && m <= lm)) {
            if (get().getMonthCompletionCount(habitId, y, m) >= habit.targetDays) {
              longest = Math.max(longest, ++current);
            } else { current = 0; }
            if (++m > 11) { m = 0; y++; }
          }
        ...
        // Daily / weekdays: consecutive calendar days
        let longest = 1; let current = 1;
        for (let i = 1; i < dates.length; i++) {
          const prev = new Date(dates[i - 1] + 'T12:00:00');
          const curr = new Date(dates[i] + 'T12:00:00');
          const diff = (curr.getTime() - prev.getTime()) / (1000 * 60 * 60 * 24);
          if (diff === 1) { longest = Math.max(longest, ++current); }
          else { current = 1; }
        }
        return longest;
      },
      ...
      markSourceCompleted: (source, date) => {
        const state = get();
        const linked = state.habits.filter(h => h.linkedSource === source && !h.archivedAt);
        linked.forEach(h => {
          if (!state.isCompleted(h.id, date)) {
            state.toggleEntry(h.id, date);
          }
        });
      },
    }),
