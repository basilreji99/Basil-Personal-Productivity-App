...
export default function Notes() {
  ...
  useEffect(() => {
    const openId = (location.state as any)?.openNoteId;
    if (!openId) return;
    ...
    <div className="bg-background min-h-screen">
      ...
      <main className="max-w-screen-xl mx-auto px-4 py-4 pb-36">
        {view === 'folders' ? (
          ...
          const isAllView = activeFolder === null;
          const pinnedNotes   = filtered.filter((n) => n.pinned);
          const unpinnedNotes = filtered.filter((n) => !n.pinned);

          // In "All" view: unfiled notes + per-folder groups; in folder view: flat unpinned list
          const unfiledNotes     = isAllView ? unpinnedNotes.filter((n) => !n.folderId) : [];
          const folderGroupsData = isAllView
            ? folders
                .map((f) => ({ folder: f, notes: unpinnedNotes.filter((n) => n.folderId === f.id) }))
                .filter((g) => g.notes.length > 0)
            : [];
          const hasBoth = pinnedNotes.length > 0 && unpinnedNotes.length > 0;
          const hasMultipleAllSections =
            pinnedNotes.length > 0 && (unfiledNotes.length > 0 || folderGroupsData.length > 0);

          const PinnedHeader = () => (
            <div className="flex items-center gap-2 mb-2 mt-1">
              <span className="material-symbols-outlined text-[13px] text-amber-500">push_pin</span>
              <p className="font-inter text-[10px] font-semibold uppercase tracking-wider text-amber-600 dark:text-amber-400">Pinned</p>
              <div className="flex-1 h-px bg-amber-200/60 dark:bg-amber-700/30" />
            </div>
          );
          const GroupHeader = ({ label, emoji, icon }: { label: string; emoji?: string; icon?: string }) => (
            <div className="flex items-center gap-2 mb-2 mt-3">
              {emoji
                ? <span className="text-sm leading-none">{emoji}</span>
                : <span className="material-symbols-outlined text-[13px] text-outline">{icon ?? 'notes'}</span>}
          ...
          const renderUnpinnedSections = (v: 'list' | 'grid' | 'masonry') => {
            ...
            if (isAllView) return (
              <>
                {unfiledNotes.length > 0 && (
                  <div>
                    {(hasMultipleAllSections || folderGroupsData.length > 0) && <GroupHeader label="Unfiled" icon="notes" />}
                    {wrap(unfiledNotes)}
                  </div>
                )}
                {folderGroupsData.map(({ folder, notes: fNotes }) => (
                  <div key={folder.id}>
                    <GroupHeader label={folder.name} emoji={folder.emoji} />
                    {wrap(fNotes)}
                  </div>
                ))}
              </>
            ...
            return (
              <>
                {hasBoth && <GroupHeader label="Notes" icon="notes" />}
                {unpinnedNotes.length > 0 && wrap(unpinnedNotes)}
              </>
            );
          };
          ...
          return (
            <>
              ...
              {renderUnpinnedSections('masonry')}
            </>
          );
        })()}
      </main>
