...
export default function Gym() {
  ...
  const { gymSessions: simpleSessions } = useFitnessStore();
  ...
  // Merge detailed (gymStore) + simple (fitnessStore) sessions for unified history
  const allHistory = useMemo(() => {
    const detailed = sortedSessions.map((s) => ({ kind: 'detailed' as const, s, sortKey: s.startedAt }));
    const simple = simpleSessions.map((s) => ({ kind: 'simple' as const, s, sortKey: s.startTime ?? (s.date + 'T12:00:00') }));
    return [...detailed, ...simple].sort((a, b) => b.sortKey.localeCompare(a.sortKey));
  }, [sortedSessions, simpleSessions]);

  const monthStats = useMemo(() => {
    const now = new Date();
    const monthStr = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`;
    const monthly = sessions.filter((s) => s.startedAt.startsWith(monthStr) && s.source !== 'health_connect');
    const simpleMonthly = simpleSessions.filter((s) => s.date.startsWith(monthStr));
    const totalVol = monthly.reduce((a, s) => a + (s.totalVolume ?? 0), 0);
    const totalSets = monthly.reduce((a, s) => a + (s.totalSets ?? 0), 0);
    return { count: monthly.length + simpleMonthly.length, volume: totalVol, sets: totalSets };
          ...
          {allHistory.length === 0 ? (
            ...
            allHistory.map((item) => {
              if (item.kind === 'simple') {
                const s = item.s;
                return (
                  <div key={s.id} className="w-full bg-surface-container-lowest rounded-2xl p-4 shadow-card">
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        <p className="font-inter font-semibold text-sm text-on-surface truncate">{s.type}</p>
                        <p className="font-inter text-xs text-outline">{fmtDate(s.date + 'T12:00:00')}</p>
                      </div>
                      <div className="flex gap-4 text-right shrink-0">
                        {s.duration ? (
                          <div>
                            <p className="font-inter font-semibold text-sm text-on-surface">{fmtDuration(s.duration)}</p>
                            <p className="font-inter text-[9px] text-outline">Duration</p>
                          </div>
                        ) : null}
                        {s.calories ? (
                          <div>
                            <p className="font-inter font-semibold text-sm text-on-surface">{s.calories}</p>
                            <p className="font-inter text-[9px] text-outline">kcal</p>
                          </div>
                        ) : null}
                      </div>
                    ...
                    {s.notes && (
                      <p className="font-inter text-[10px] text-outline mt-1.5 truncate">{s.notes}</p>
                    )}
                  </div>
                );
              }
