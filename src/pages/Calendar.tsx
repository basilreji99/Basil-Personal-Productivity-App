...
export default function Calendar() {
  const {
    profile, clientId, setClientId, clientSecret, setClientSecret,
    clearAuth, startAuth,
        ...
        {showAccounts && (
          <div className="bg-surface-container rounded-2xl p-4 space-y-3">
            ...
            {profile ? (
              ...
              <div className="space-y-2">
                <p className="font-inter text-[10px] font-semibold text-on-surface-variant uppercase tracking-wide px-0.5">Google credential (Drive sync)</p>
                <div className="relative">
                  <input
                    ...
                    className="w-full bg-surface-container-low border border-outline-variant rounded-xl px-3 py-2 pr-9 font-inter text-sm text-on-surface placeholder:text-on-surface-variant outline-none focus:border-primary"
                  />
                  {inputId && (
                    <button
                      onClick={() => setInputId('')}
                ...
                {showSetup && (
                  <div className="bg-surface-container-low rounded-xl p-3 space-y-2 text-xs font-inter text-on-surface-variant">
                    ...
                    <ol className="list-decimal list-inside space-y-1 leading-relaxed">
                      <li>Go to <span className="text-primary">console.cloud.google.com</span> → select your project</li>
                      <li>Enable <strong className="text-on-surface">Calendar API</strong> and <strong className="text-on-surface">Drive API</strong></li>
                      <li>OAuth consent screen → External → add your email as test user → add scopes: <code className="bg-surface-container px-1 rounded">drive.appdata</code> <code className="bg-surface-container px-1 rounded">calendar.readonly</code></li>
                      <li>Credentials → <strong className="text-on-surface">Web application</strong> → add <code className="bg-surface-container px-1 rounded text-[9px]">http://localhost:5173/</code> and your relay URL under Authorized redirect URIs</li>
                      <li>Paste Client ID + Secret above, then tap Save &amp; Sign In</li>
                    </ol>
                  </div>
                )}
              </div>
